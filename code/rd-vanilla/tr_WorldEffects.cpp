...
void CSnowSystem::Render(void)
{
	...
	float vidscale = sqrt((float)glConfig.vidWidth / SCREEN_WIDTH * (float)glConfig.vidHeight / SCREEN_HEIGHT);

	if (qglPointParameterfEXT)
	{
		qglPointSize(10.0 * vidscale);
		qglPointParameterfEXT(GL_POINT_SIZE_MIN_EXT, 1.0 * vidscale);
		qglPointParameterfEXT(GL_POINT_SIZE_MAX_EXT, 4.0 * vidscale);
