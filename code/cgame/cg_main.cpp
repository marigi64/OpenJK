...
//	int				tag;
	...
	for (iconCnt=0;iconCnt<sideRightIconCnt;i++)
	{
		...
		if (inv_icons[i])
		{
			...
			cgi_R_SetColor(colorTable[CT_ICON_BLUE]);
			CG_DrawNumField (holdX + addX, y + smallIconSize * cgs.widthRatioCoef, 2, cg.snap->ps.inventory[i], 6, 12,
				NUM_FONT_SMALL,qfalse);
...
void CG_DrawDataPadInventorySelect( void )
{
	...
	for (iconCnt=0;iconCnt<sideLeftIconCnt;i--)
	{
		...
		if (inv_icons[i])
		{
			...
			CG_DrawPic( holdX, graphicYPos+10, smallIconSize * cgs.widthRatioCoef, smallIconSize, inv_icons[i] );

			cgi_R_SetColor(colorTable[CT_ICON_BLUE]);
			CG_DrawNumField (holdX + addX, graphicYPos + smallIconSize, 2 * cgs.widthRatioCoef, cg.snap->ps.inventory[i], 6, 12,
				NUM_FONT_SMALL,qfalse);
	...
	{
		cgi_R_SetColor(colorTable[CT_WHITE]);
		CG_DrawPic( centerXPos-(bigIconSize/2) * cgs.widthRatioCoef, (graphicYPos-((bigIconSize-smallIconSize)/2))+10, bigIconSize * cgs.widthRatioCoef, bigIconSize, inv_icons[cg.DataPadInventorySelect] );
		addX = (float) bigIconSize * .75;
		cgi_R_SetColor(colorTable[CT_ICON_BLUE]);
		CG_DrawNumField ((centerXPos-(bigIconSize/2)) + addX, graphicYPos, 2 * cgs.widthRatioCoef, cg.snap->ps.inventory[cg.DataPadInventorySelect], 6, 12,
			NUM_FONT_SMALL,qfalse);
...
*/
	...
	{
		CG_DrawPic( x-(bigIconSize/2) * cgs.widthRatioCoef, (y-((bigIconSize-smallIconSize)/2)) + yOffset, bigIconSize * cgs.widthRatioCoef, bigIconSize, force_icons[showPowers[cg.forcepowerSelect]] );
	}
...
//	memcpy(fadeColor, colorTable[CT_WHITE], sizeof(vec4_t));
	...
	for (iconCnt=1;iconCnt<(sideLeftIconCnt+1);i--)
	{
		...
		{
			CG_DrawPic( holdX, graphicYPos, smallIconSize * cgs.widthRatioCoef, smallIconSize, force_icons[showDataPadPowers[i]] );
		}
	...
	if (force_icons[showDataPadPowers[cg.DataPadforcepowerSelect]])
	{
		...
		cgi_R_SetColor(colorTable[CT_WHITE]);
		CG_DrawPic( centerXPos-(bigIconSize/2) * cgs.widthRatioCoef, (graphicYPos-((bigIconSize-smallIconSize)/2)), bigIconSize * cgs.widthRatioCoef, bigIconSize, force_icons[showDataPadPowers[cg.DataPadforcepowerSelect]] );
		...
		{
			CG_DrawPic( centerXPos-(bigIconSize/2) * cgs.widthRatioCoef, (graphicYPos-((bigIconSize-smallIconSize)/2)), bigIconSize * cgs.widthRatioCoef, bigIconSize, cgs.media.DPForcePowerOverlay );
		}
	}
